...
### 1.2 Dockerfile — ключевые инструкции

| Инструкция  | Что делает |
|-------------|------------|
| `FROM`      | Базовый образ. Первая строка в Dockerfile |
| `COPY`      | Копирует файлы с хоста в образ |
| `ADD`       | Как COPY, но умеет распаковывать архивы и скачивать URL |
| `RUN`       | Выполняет команду при сборке образа (создаёт слой) |
| `CMD`       | Команда по умолчанию при запуске контейнера (можно переопределить в `docker run`) |
| `ENTRYPOINT`| Точка входа (труднее переопределить, объединяется с CMD) |
| `EXPOSE`    | Документирует порт (не открывает его, нужен `-p`) |
| `ENV`       | Переменная окружения в образе |
| `ARG`       | Аргумент сборки (доступен только при `docker build`) |
| `WORKDIR`   | Рабочая директория |
| `USER`      | От какого пользователя запускать процесс |

**CMD vs ENTRYPOINT:**
```dockerfile
# CMD — можно полностью заменить: docker run img echo hello
CMD ["./app"]
